...
function Dashboard() {
  ...
  const [compartments, setCompartments] = React.useState([
    { label: 'Compartment A', active: false, lastUpdated: '--' },
    { label: 'Compartment B', active: false, lastUpdated: '--' },
    { label: 'Compartment C', active: false, lastUpdated: '--' },
  ]);
...
{activeTab === 'Recyclable' && <>
  ...
  <div style={{ display: 'flex', gap: '16px' }}>
    {[
      { label: 'Total Waste Collected', value: '--', unit: 'kg', change: '--', icon: (
      ...
      { label: 'Recyclable Waste', value: '--', unit: 'kg', change: '--', icon: (
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2d5a27" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M7 19H4.815a1.83 1.83 0 0 1-1.57-.881 1.785 1.785 0 0 1-.004-1.784L7.196 9.5"/>
          <path d="M11 19h8.203a1.83 1.83 0 0 0 1.556-.89 1.784 1.784 0 0 0 0-1.775l-1.226-2.12"/>
          <path d="m14 16-3 3 3 3"/>
      ...
      <div key={i} style={{
        ...
        <div>
          ...
          <div style={{ fontSize: '14px', color: '#4CAF50', marginTop: '4px' }}>{card.change}</div>
        </div>
        <span>{card.icon}</span>
      </div>
    ))}
  ...
  <div style={{ display: 'flex', gap: '12px' }}>
    {[
      { label: 'Paper Waste', value: '--', unit: 'kg', change: '--' },
      { label: 'Plastic Waste', value: '--', unit: 'kg', change: '--' },
      { label: 'Glass Waste', value: '--', unit: 'kg', change: '--' },
      { label: 'Metal Waste', value: '--', unit: 'kg', change: '--' },
    ].map((card, i) => (
      <div key={i} style={{
        ...
        <div style={{ fontSize: '25px', fontWeight: '800', color: '#1a3a1a' }}>
          {card.value} <span style={{ fontSize: '13px', fontWeight: '400' }}>{card.unit}</span>
        </div>
        <div style={{ fontSize: '12px', color: '#4CAF50', marginTop: '4px' }}>{card.change}</div>
      </div>
    ))}
  </div>
    ...
    <div style={{ display: 'flex', gap: '12px' }}>
      {compartments.map((comp, i) => (
        <div key={i} style={{
          ...
          boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
          border: `1px solid ${comp.active ? '#4CAF50' : '#ffcccc'}`,
        }}>
          <div>
            <div style={{ fontSize: '13px', fontWeight: '600', color: '#333' }}>{comp.label}</div>
            <div style={{ fontSize: '11px', color: '#999', marginTop: '2px' }}>Last updated: {comp.lastUpdated}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <div style={{
              width: '8px', height: '8px', borderRadius: '50%',
              background: comp.active ? '#4CAF50' : '#ff4444',
              boxShadow: comp.active ? '0 0 6px #4CAF50' : 'none',
            }} />
            <span style={{
              padding: '4px 12px', borderRadius: '20px',
              background: comp.active ? '#4CAF50' : '#ff4444',
              color: '#fff', fontSize: '11px', fontWeight: '700',
            }}>{comp.active ? 'Active' : 'Inactive'}</span>
          </div>
        </div>
      ))}
    </div>
...
{activeTab === 'Non-Biodegradable' && <>
  ...
  <div style={{ display: 'flex', gap: '16px' }}>
    <div style={{
      ...
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#795548" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M10 11v6"/><path d="M14 11v6"/>
        <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/>
        <path d="M3 6h18"/>
        <path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
